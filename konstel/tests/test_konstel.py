...
def test_sars2_generate_length():
    result = konstel.generate(
        "sars-cov-2-s.protein", f"{data_dir}/spike.prot.fa", length=21
    )
...
def test_sars2_regen():
    result = konstel.regenerate(
        "sars-cov-2-s", "0k8n9hjh5xh5kbef1k6ye7e2d4brhpry5r985avrtf69v6amrbc0"
    )
    assert result["id"] == "S:huhijig-akih"
...
def test_sars2_regen_directive():
    result = konstel.regenerate(
        "sars-cov-2-s.protein", "0k8n9hjh5xh5kbef1k6ye7e2d4brhpry5r985avrtf69v6amrbc0"
    )
    assert result["id"] == "S:huhijig-akih"
...
def test_sars2_regen_prefix():
    result = konstel.regenerate(
        "sars-cov-2-s", "S:0k8n9hjh5xh5kbef1k6ye7e2d4brhpry5r985avrtf69v6amrbc0"
    )
    assert result["id"] == "S:huhijig-akih"
...
def test_sars2_legacy_regen():
    result = konstel.regenerate(
        "sars-cov-2-s-legacy", "mfcqn6mh3bnp7vv6eirptvbqik5c65ip"
    )
...
def test_sars2_legacy_regen_prefix():
    result = konstel.regenerate(
        "sars-cov-2-s-legacy", "S:mfcqn6mh3bnp7vv6eirptvbqik5c65ip"
    )
    assert result["id"] == "S:papoheme"
...
def test_sars2_regen_length():
    result = konstel.regenerate(
        "sars-cov-2-s",
        "S:0k8n9hjh5xh5kbef1k6ye7e2d4brhpry5r985avrtf69v6amrbc0",
        length=21,
    )
    assert result["id"] == "S:huhijig-akihifu-tofikip"
...
def test_sars2_legacy_protein():
    result = konstel.generate(
        "sars-cov-2-s-legacy.protein", f"{data_dir}/spike.prot.fa"
    )
    assert result["id"] == "S:papoheme"
...
def test_sars2_legacy_genome():
    result = konstel.generate(
        "sars-cov-2-s-legacy.genome", f"{data_dir}/spike.genome.fa"
    )
    assert result["id"] == "S:papoheme"
